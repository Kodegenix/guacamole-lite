...
class GuacdClient {
    ...
    constructor(server, clientConnection) {
        if (clientConnection.log) {
            this.log = clientConnection.log;
        }
    ...
    close(error) {
        ...
        if (this.log) {
            ...
            this.log.info('Closing guacd connection');
        }
    ...
    processConnectionOpen() {
        if (this.log) {
            this.log.info('guacd connection open');
            this.log.info('Selecting connection type: ' + this.clientConnection.connectionType);
        }
    ...
    sendHandshakeReply() {
        ...
        if (this.log) {
            this.log.debug('Server sent handshake: ' + serverHandshake);
        }
    ...
    sendOpCode(opCode) {
        ...
        if (this.log) {
            this.log.debug('Sending opCode: ' + opCode);
        }
        this.send(opCode);
    }
